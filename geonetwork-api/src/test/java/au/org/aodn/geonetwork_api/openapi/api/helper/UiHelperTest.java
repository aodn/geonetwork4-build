...
public class UiHelperTest {
    ...
    public void verifyDeepMergeKeepsSiblings() {
        ...
        when(api.getUiConfiguration(eq(UiHelper.UI_IDENTIFIER))).thenReturn(live);
        ...
        Mockito.verify(api).updateUiConfiguration(eq(UiHelper.UI_IDENTIFIER), captor.capture());
    ...
    public void verifyApiFailureReportedAsError() {
        UiApi api = Mockito.mock(UiApi.class);
        when(api.getUiConfiguration(eq(UiHelper.UI_IDENTIFIER)))
